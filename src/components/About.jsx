import React from 'react'

const About = () => {
    return (
        <>
            <section  id='about'>
<div className='bg-blue-400 shadow-lg mt-10 mx-auto w-full rounded-md flex-col gap-2'>

                <h1 className='text-3xl font-mono font-semibold text-center pt-4'>Hii,I'm Ritik Pandey.Nice to meet you.</h1>
                <h3 className='mt-4 text-center text-slate-100  text-xl'>
                    Currently a student,Persuing Diploma in Information Technology  at Gp kashipiur,Uttarakhand.
                </h3>
                <h3 className='text-center  text-slate-100  text-xl'>
                    developing scalable frontend using HTML, CSS and React.js.
                </h3>
                <h3  className='text-center  text-slate-100  text-xl'>
                I'm a firm believer in continuous learning,
                </h3>
                <h3  className='text-center  text-slate-100  text-xl'>
                 and I always strive to stay at the forefront of web development trends.
                </h3>
                <h3 className='text-center  text-slate-100 text-xl pb-4'>
                    Loves to code(obviously),exlporing new places (a lot sometimes),being creative.
                </h3>
</div>

            </section>
        </>
    )
}

export default About
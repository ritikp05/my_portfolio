...
const Projects = ({ name, tech, desc, github, live, oddeven }) => {
  ...
  return (
    ...
    <motion.div initial={oddeven?{ x: ['80vw'],opacity:0}:{ x: ['-80vw'],opacity:0}}
...
return(
    ...
    <button className='px-2 border-2 cursor-default border-green-500 hover:bg-green-500 hover:text-white rounded-md ' key={index}>{elem}</button>
      ...
      <h1 className='flex-wrap ml-5 mt-1 lg:text-xl md:text-xl sm:text-lg cursor-default '>{desc}</h1>
      <div className='flex gap-6 mt-2 ml-8 '>
        <a href={github} target='_blank' className='ml-10'><VscGithubInverted className='text-3xl' />
        </a>
        <a href={live} target='_blank'><IoGlobeOutline className='text-3xl text-blue-600' /></a>
      </div>
